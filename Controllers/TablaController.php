...
switch($opcion){
    ...
    case 3:
            ...
            $consulta = "UPDATE usuarios SET baja = 1 WHERE id='$id'";		
            $resultado = $conexion->prepare($consulta);
            $resultado->execute(); 
    ...
    case 4:    
        $consulta = "SELECT * FROM departamentos inner join usuarios on departamentos.id = usuarios.id_departamento  WHERE baja = '0'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
